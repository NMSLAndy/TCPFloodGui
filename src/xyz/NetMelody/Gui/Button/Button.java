...
public class Button {
	...
	private SlickColor color;
	private ArrayList<Circle> circles = new ArrayList<Circle>();
	private boolean clicked = false;
	
	public Button(int id, String text, int x, int y, int width, int height, boolean enabled, Action action, SlickColor color) {
		this.id = id;
		this.text = text;
		this.x = x;
		this.y = y;
	...
	public void draw(boolean withTick) {
		...
		RenderUtil.drawFastRoundedRect(this.getX(), this.getY() - 4, this.getX() + this.getWidth(), this.getY() + this.getHeight() - 6, 2,
				this.isEnabled()
						? (this.isHovered()
		...
		Client.core.font.s12.drawString(
				...
				this.getY() + this.getHeight() / 2 - (Client.core.font.s12.getHeight(this.getText()) / 2) - 4, this.getText(),
				this.isEnabled() ? this.getColor() : new SlickColor(120, 120, 120));
