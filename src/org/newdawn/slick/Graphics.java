...
public class Graphics {
	...
	private static int DEFAULT_SEGMENTS = 50;
	...
	/** The current color */
	private SlickColor currentColor = SlickColor.white;
	...
	public void clearAlphaMap() {
		...
		setColor(new SlickColor(0,0,0,0));
	 ...
	 */
	public void setBackground(SlickColor color) {
		predraw();
	 ...
	 */
	public SlickColor getBackground() {
		predraw();
		FloatBuffer buffer = BufferUtils.createFloatBuffer(16);
		GL.glGetFloat(SGL.GL_COLOR_CLEAR_VALUE, buffer);
		postdraw();

		return new SlickColor(buffer);
	}
	 ...
	 */
	public void setColor(SlickColor color) {
		if (color == null) {
			return;
		}
		
		currentColor = new SlickColor(color);
		predraw();
		currentColor.bind();
		postdraw();
	}
	 ...
	 */
	public SlickColor getColor() {
		return new SlickColor(currentColor);
	}
	 ...
	 */
	public void drawImage(Image image, float x, float y, SlickColor col) {
		predraw();
		image.draw(x, y, col);
		currentColor.bind();
		postdraw();
	...
	public void drawAnimation(Animation anim, float x, float y) {
		drawAnimation(anim, x, y, SlickColor.white);
	}
	 ...
	 */
	public void drawAnimation(Animation anim, float x, float y, SlickColor col) {
		predraw();
		anim.draw(x, y, col);
		currentColor.bind();
		postdraw();
	...
	public void drawImage(Image image, float x, float y) {
		drawImage(image, x, y, SlickColor.white);
	}
	 ...
	 */
	public SlickColor getPixel(int x, int y) {
		predraw();
		GL.glReadPixels(x, screenHeight - y, 1, 1, SGL.GL_RGBA,
				SGL.GL_UNSIGNED_BYTE, readBuffer);
		postdraw();

		return new SlickColor(translate(readBuffer.get(0)), translate(readBuffer
	 ...
	 */
	public void drawImage(Image image, float x, float y, float x2, float y2,
			float srcx, float srcy, float srcx2, float srcy2, SlickColor col) {
		predraw();
	 ...
	 */
	public void drawImage(Image image, float x, float y, float srcx,
			float srcy, float srcx2, float srcy2, SlickColor col) {
		drawImage(image, x, y, x + image.getWidth(), y + image.getHeight(),
	 ...
	 */
	public void drawGradientLine(float x1, float y1, SlickColor Color1, float x2,
								 float y2, SlickColor Color2) {
		predraw();
