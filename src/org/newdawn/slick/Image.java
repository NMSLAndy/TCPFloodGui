...
public class Image implements Renderable {
	...
	public static int FILTER_NEAREST = 2;
	 ...
	 */
	public Image(String ref, SlickColor trans) throws SlickException  {
	 ...
	 * @throws SlickException Indicates a failure to load the image
	 */
	public Image(String ref, boolean flipped, int f, SlickColor transparent) throws SlickException {
	...
	public void setColor(int corner, float r, float g, float b, float a) {
		if (corners == null) {
			corners = new SlickColor[] {new SlickColor(1,1,1,1f),new SlickColor(1,1,1,1f), new SlickColor(1,1,1,1f), new SlickColor(1,1,1,1f)};
		}
	 ...
	 * @throws SlickException Indicates a failure to load the image
	 */
	private void load(InputStream in, String ref, boolean flipped, int f, SlickColor transparent) throws SlickException {
		this.filter = f == FILTER_LINEAR ? SGL.GL_LINEAR : SGL.GL_NEAREST;
	 ...
	 */
	public void draw(float x, float y, SlickColor filter) {
		init();
	...
	public void draw(float x,float y,float scale) {
		init();
		draw(x,y,width*scale,height*scale,SlickColor.white);
	}
	 ...
	 */
	public void draw(float x,float y,float scale,SlickColor filter) {
		init();
	...
	public void draw(float x,float y,float width,float height) {
		init();
		draw(x,y,width,height,SlickColor.white);
	}
    ...
    public void drawSheared(float x,float y, float hshear, float vshear) { 
    	this.drawSheared(x, y, hshear, vshear, SlickColor.white);
    }
	/**
	 * Draw this image at a specified location and size
	 * 
	 ...
	 */
    public void drawSheared(float x,float y, float hshear, float vshear, SlickColor filter) { 
    	if (alpha != 1) {
    		if (filter == null) {
    			filter = SlickColor.white;
    		}
    		
    		filter = new SlickColor(filter);
    		filter.a *= alpha;
    	}
        if (filter != null) { 
            filter.bind(); 
	 ...
	 */
    public void draw(float x,float y,float width,float height,SlickColor filter) { 
    	if (alpha != 1) {
    		if (filter == null) {
    			filter = SlickColor.white;
    		}
	...
	public void drawFlash(float x,float y,float width,float height) {
		drawFlash(x,y,width,height,SlickColor.white);
	}
	 ...
	 */
	public void drawFlash(float x,float y,float width,float height, SlickColor col) {
		init();
	...
	public void draw(float x, float y, float x2, float y2, float srcx, float srcy, float srcx2, float srcy2) {
		draw(x,y,x2,y2,srcx,srcy,srcx2,srcy2,SlickColor.white);
	}
	 ...
	 */
	public void draw(float x, float y, float x2, float y2, float srcx, float srcy, float srcx2, float srcy2, SlickColor filter) {
		init();
...
//		GL.glEnd();
	 ...
	 * @param filter The colour filter to apply when drawing
	 */
	public void drawEmbedded(float x, float y, float x2, float y2, float srcx, float srcy, float srcx2, float srcy2, SlickColor filter) {
		if (filter != null) {
			filter.bind();
		}
	 ...
	 */
	public SlickColor getColor(int x, int y) {
		if (pixelData == null) {
		...
		if (texture.hasAlpha()) {
			return new SlickColor(translate(pixelData[offset]),translate(pixelData[offset+1]),
