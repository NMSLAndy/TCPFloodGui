...
public class Gradient {
	...
	public void genImage() {
		if (image == null) {
			...
			for (int i=0;i<128;i++) {
				SlickColor col = getColorAt(i / 128.0f);
				for (int j=0;j<16;j++) {
	 ...
	 */
	public void addStep(float location, SlickColor c) {
	 ...
	 */
	public SlickColor getColorAt(float p) {
		if (p <= 0) {
		...
		for (int i=1;i<steps.size();i++) {
			...
			if (p <= current.location) {
				...
				SlickColor c = new SlickColor(1,1,1,1);
	...
	private class Step {
		 ...
		 */
		public Step(float location, SlickColor c) {
